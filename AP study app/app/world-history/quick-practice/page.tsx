...
import { apWorldHistoryQuestions } from "../../data/apWorldHistoryQuestions"

export default function QuickPracticePage() {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0)
  const [correctAnswers, setCorrectAnswers] = useState(0)
  const [incorrectAnswers, setIncorrectAnswers] = useState(0)
  const [questions] = useState(() => apWorldHistoryQuestions.sort(() => 0.5 - Math.random()).slice(0, 20))
  const [quizComplete, setQuizComplete] = useState(false)

  useEffect(() => {
    const storedProgress = getProgress("world-history", "quick-practice")
    const storedCorrect = getProgress("world-history", "quick-practice-correct") || 0
    const storedIncorrect = getProgress("world-history", "quick-practice-incorrect") || 0
  ...
  const handleAnswered = (isCorrect: boolean) => {
    if (isCorrect) {
      ...
      saveProgress("world-history", "quick-practice-correct", correctAnswers + 1)
    } else {
      setIncorrectAnswers((prev) => prev + 1)
      saveProgress("world-history", "quick-practice-incorrect", incorrectAnswers + 1)
    }

    if (currentQuestionIndex < 19) {
      setCurrentQuestionIndex((prev) => prev + 1)
      saveProgress("world-history", "quick-practice", currentQuestionIndex + 1)
    } else {
  ...
  const restartQuiz = () => {
    ...
    saveProgress("world-history", "quick-practice", 0)
    saveProgress("world-history", "quick-practice-correct", 0)
    saveProgress("world-history", "quick-practice-incorrect", 0)
  }
